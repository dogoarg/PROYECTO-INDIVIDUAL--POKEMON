...
function rootReducer(state = initialState, action) {
    switch (action.type) {
        ...
        case 'FILTER_BY_TYPES':
            ...
            const typesFiltered = action.payload === 'ALL' ? allPokemons : allPokemons.filter(el => {
                // console.log(el);
                return el.types?.includes(action.payload) || el.tipos?.includes(action.payload)
            })
        ...
        case 'FILTER_CREATED':
            ...
            const createdFilter = action.payload === 'created' ? allPokemon.filter(el => el.createdInDb) : allPokemon.filter(el => {
                if (el.createdInDb) {
