...
const Detail = (props) => {
    ...
    return (
        <div className='home-container '>
            <Link className='textInicio' to= '/home'>Inicio</Link>
            {pokemon && pokemon !== "no se a encontrado ningun pokemon" ?
                <div className='detailPokemon2'>
                    <h1>{pokemon.name}</h1>
                    <img src={pokemon.img} alt="img not found" width="200px" height="250px" />
                    <h2 className='caracteristicas'>ID: {pokemon.id}</h2>
                    <div>
                        <h2 className='caracteristicas'>Tipos: {pokemon.tipos?.map(tipo => <span className="tipos">{tipo}</span>)} {pokemon.types?.map(tipo => <span className="tipos">{tipo}</span>)}</h2>
                    </div>
