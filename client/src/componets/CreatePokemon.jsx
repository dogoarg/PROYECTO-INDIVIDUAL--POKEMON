...
const CreatePokemon = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setError(null)
        if (validate()) {
            const res = await axios.post("http://localhost:3001/pokemons", newPokemon)
            if(res.status === 200){
        ...
        <div className='create-container '>
            ...
            <form  onSubmit={handleSubmit}>
                ...
                <div>
                    <h3>Tipos:</h3>
                    <div>
                        {newPokemon.tipo.map(t => <div>{t} <span className='cursorPointer xEliminar' onClick={() => eliminarTipo(t)}>X</span></div>)}
                    </div>
                </div>
